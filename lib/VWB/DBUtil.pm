...
use constant DEFAULT_USE_PROXY_ACCOUNT => __DEFAULT_USE_PROXY_ACCOUNT__;
...
sub getInstance {
    ...
    if (!defined($instance)){
        $instance = new VWB::DBUtil(@_);
        if (!defined($instance)){
            confess "Could not instantiate VWB::DBUtil";
        }
    }
    return $instance;
}
...
=head1 SYNOPSIS
 ...
 my $dbutil = VWB::DBUtil::getInstance(username    => $username, 
                                       password    => $password,
                                       server      => $server,
