...
B<Description:> Instantiate VWB::Logger object

B<Parameters:> 

 %args

B<Returns:> Returns a reference to VWB::Logger
...
sub _initConfiguration {
    ...
    my $conf = q(
    log4perl.logger                    = sub {return VWB::Logger::_getLogLevel();}

    log4perl.appender.Logfile          = Log::Log4perl::Appender::File
    log4perl.appender.Logfile.filename = sub { return VWB::Logger::_getLogfile(); }
...
sub getInstance {
    ...
    if (!defined($instance)){
        ...
        $instance = new VWB::Logger(@_);

        if (!defined($instance)){
            confess "Could not instantiate VWB::Logger";
        }
    }
...
B<Description:> VWB::Logger class destructor
